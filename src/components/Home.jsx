...
import { Shield, FileCheck, AlertTriangle, CheckCircle, Clock, ArrowRight } from '@/lib/icons';
...
const VulnerabilityCard = ({ data, vulnerabilities }) => {
  // let mockVulnerabilities = data
...
const Home = ({ vulnerabilities, coreFileScan, status }) => {
  ...
  return (
    <div className="w-full bg-secondary p-4">
      ...
      {JSON.stringify(lastChecks)}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {lastChecks.loading ? <Loading/> : <VulnerabilityCard data={mockVulnerabilities} check={lastChecks.last_core_files_check}/>}
        {lastChecks.loading ? <Loading/> : <CoreFilesCard data={mockCoreFiles} check={lastChecks.last_vulnerability_check} />}
      </div>
    </div>
  );
};
